...
use crate::{StatsMap, syscall_table};
...
/// FR6.1-6.3: live-updating single Syscalls panel, sortable, with a summary header.
///
/// `poll_events` is called once per loop tick (~every 50ms) to drain the ring buffer
/// and update `stats` before the next redraw; keeping ring-buffer/eBPF specifics out
/// of this module. Restores the terminal on every exit path (including Ctrl+C/'q')
/// so no eBPF probes are left attached to a hung terminal session.
pub fn run(
    pid: u32,
    refresh: Duration,
    stats: StatsMap,
    ...
    let result = run_loop(&mut terminal, pid, refresh, stats, &mut poll_events);
...
fn run_loop(
    ...
    loop {
        ...
        if last_draw.elapsed() >= refresh {
            let rows: Vec<(u64, crate::SyscallStats)> = stats.borrow().iter().map(|(&nr, &s)| (nr, s)).collect();
            draw(terminal, pid, start.elapsed(), sort_by, rows)?;
...
fn draw(
    ...
    terminal.draw(|frame| {
        ...
        let header_text = format!(
            "PID {pid}  |  uptime {}s  |  {total_events} syscalls seen  |  sorted by {sort_label} ('s' to toggle)",
            uptime.as_secs(),
        );
        frame.render_widget(
